...
impl BlockAllocator {
    pub(crate) const unsafe fn new(start: StartEnd) -> Self {
        Self {
            inner: UnsafeCell::new(unsafe { BlockAllocatorInner::new(start) }),
        }
    }
    ...
    pub unsafe fn alloc(&self, layout: Layout) -> Option<NonNull<u8>> {
        unsafe { self.with_inner(|inner| inner.alloc(layout)) }
    }
    ...
    pub unsafe fn grow(
        ...
        unsafe { self.with_inner(|inner| inner.grow(ptr, layout, new_layout)) }
    }
}
...
impl BlockAllocatorInner {
    ...
    pub unsafe fn alloc(&mut self, layout: Layout) -> Option<NonNull<u8>> {
        ...
        loop {
            match list_ptr {
                &mut Some(mut current_block) => {
                    if let Some(alloc) = unsafe { Self::allocate_into_block(list_ptr, full_layout) }
                    {
                        return Some(alloc);
                    }
                    list_ptr = unsafe { &mut current_block.as_mut().next };
                }
            ...
            if let Some(list_to_block) = next_block {
                let is_block_directly_after = {
                    if let Some(block) = list_to_block {
                        core::ptr::eq(
                            block.0.as_ptr(),
                            ptr.add(either_layout_initial.size()).cast(),
                        )
                    } else {
                        false
                    }
    ...
    unsafe fn is_block_at_end(&self, ptr: *mut u8, total_layout: Layout) -> bool {
        unsafe { self.inner_allocator.tip() == NonNull::new(ptr.add(total_layout.size())) }
    }
