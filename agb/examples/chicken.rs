...
impl Chicken {
    ...
    fn restrict_to_screen(&mut self) {
        if self.position.x > (WIDTH * 5 - 8 + 4).into() {
            self.velocity.x = 0.into();
            self.position.x = (WIDTH * 5 - 8 + 4).into();
...
include_background_gfx!(background, "7e8dd2", map => deduplicate "examples/gfx/chicken-map.aseprite");
