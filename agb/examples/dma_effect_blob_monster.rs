...
fn main(mut gba: agb::Gba) -> ! {
    ...
    let mut frame_idx = Num::<i32, 8>::new(0);
    ...
    loop {
        ...
        monster.position =
            vec2(WIDTH / 2, HEIGHT / 2) + (vec2(frame_idx.sin(), frame_idx.cos()) * 10).trunc();
        frame.commit();
    }
}
...
impl BlobMonster {
    ...
    pub fn new() -> Self {
        let blob_monster_widths = (0..Self::BLOB_MONSTER_HEIGHT)
            .map(|y| Num::new(y).sqrt() * 10)
            .collect::<Vec<Num<i32, 12>>>();
        ...
        let width_starts_pairs = (0..160)
            .map(|y| {
                if y < self.position.y {
                    ...
                    let width = self.widths[monster_y as usize];
                    let x = Num::new(self.position.x) - width / 2;
