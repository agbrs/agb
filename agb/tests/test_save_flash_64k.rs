...
const MAGIC: [u8; 32] = *b"agb-test-flash64k_______________";
...
fn save_setup(gba: &mut agb::Gba) -> SaveSlotManager<TestMetadata> {
    ...
    if INITIALIZED.swap(true, Ordering::SeqCst) {
        ...
        gba.save
            ...
            .expect("Failed to reopen Flash 64K")
    } else {
        // First call, use init
        gba.save
            .init_flash_64k(NUM_SLOTS, MAGIC, MIN_SECTOR_SIZE, Some(timers.timer2))
            .expect("Failed to init Flash 64K")
    }
}
